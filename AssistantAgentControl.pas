...
uses
  ...
  ABase, AControlEx, ALogRichEditControl, ATypes,
  AclConsts, AclMessageImpl;

function AssistantAgentControl_AddToLog(LogGroup: TLogGroupMessage; LogType: TLogTypeMessage;
    const StrMsg: WideString): AInt;

function AssistantAgentControl_Init(ts: TWinControl; OnSendMessage: TProcMessageStr): AError;
...
begin
  if Assigned(FAgentControl) then
    Result := FAgentControl.AddToLog(LogGroup, LogType, StrMsg);
end;

function AssistantAgentControl_Init(ts: TWinControl; OnSendMessage: TProcMessageStr): AError;
begin
  try
    FAgentControl := TAgentControl.Create();
    FAgentControl.OnSendMessage := OnSendMessage;
    FAgentControl.Control := ts;
    FAgentControl.Initialize();
  except
    FAgentControl := nil;
  end;
end;
